...
// QueryHotspot chains the current query on the "hotspot" edge.
func (_q *NotificationQuery) QueryHotspot() *HotspotQuery {
...
func (_q *NotificationQuery) Clone() *NotificationQuery {
	...
	return &NotificationQuery{
		...
		withHotspot: _q.withHotspot.Clone(),
...
// WithHotspot tells the query-builder to eager-load the nodes that are connected to
// the "hotspot" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *NotificationQuery) WithHotspot(opts ...func(*HotspotQuery)) *NotificationQuery {
	query := (&HotspotClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withHotspot = query
	return _q
}
...
func (_q *NotificationQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Notification, error) {
	...
	if query := _q.withHotspot; query != nil {
		if err := _q.loadHotspot(ctx, query, nodes, nil,
			func(n *Notification, e *Hotspot) { n.Edges.Hotspot = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *NotificationQuery) loadHotspot(ctx context.Context, query *HotspotQuery, nodes []*Notification, init func(*Notification), assign func(*Notification, *Hotspot)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*Notification)
	for i := range nodes {
		if nodes[i].HotspotID == nil {
			continue
		}
		fk := *nodes[i].HotspotID
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "hotspot_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *NotificationQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withHotspot != nil {
			_spec.Node.AddColumnOnce(notification.FieldHotspotID)
		}
	}

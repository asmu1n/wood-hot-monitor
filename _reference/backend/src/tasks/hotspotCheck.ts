...
export async function runHotspotCheck(io: Server): Promise<void> {
    ...
    let globalAICallCount = 0; // 全局 AI 调用计数
    ...
    for (const keyword of keywords) {
        ...
        try {
            ...
            console.log(
                `  Total: ${allResults.length} raw → ${uniqueResults.length} unique → ${freshResults.length} fresh (within ${MAX_AGE_HOURS}h)`
            );
            ...
            const TWITTER_QUOTA = 1; // 进一步截取消息：从 3 -> 1
            const OTHER_QUOTA = 1; // 进一步截取消息：从 2 -> 1
            ...
            for (const item of sortedResults) {
                ...
                // 检查全局上限 (确保单次抓取不超过 12 次 AI 调用)
                if (globalAICallCount >= 12) {
                    break;
                }

                try {
                    // 检查是否已存在
                    const existing = await db.query.hotspots.findFirst({
                        where: and(eq(hotspotsTable.url, item.url), eq(hotspotsTable.source, item.source))
                    });
                    ...
                    // AI 分析前置过滤：
                    // 如果关键词没有被提及，且不是该来源的前 3 条结果，则跳过 AI 以节省额度
                    const fullText = item.title + '\n' + item.content;
                    const preMatch = preMatchKeyword(fullText, expandedKeywords);
                    const isTopResult = (item.source === 'twitter' ? twitterProcessed : otherProcessed) < 3;

                    if (!preMatch.matched && !isTopResult) {
                        console.log(`  ⏭ Skipping AI analysis (no match & not top result): ${item.title.slice(0, 30)}...`);
                        continue;
                    }

                    globalAICallCount++; // 增加全局计数
